// ═══════════════════════════════════════════════
// 창업지도 — AI 창업 분석 요약 (Vercel Serverless)
...
function generateMockSummary({ bizType, areaName, riskScore, roi, bep, competitionScore, regulationGrade, closeRate, survivalRate }) {
  ...
  if (competitionScore !== null && competitionScore < 50) {
    risks.push({ level: 'high', title: '경쟁 포화', desc: `반경 500m 내 동종 업소가 밀집해 있습니다. 메뉴·서비스·인테리어 차별화 없이는 생존이 어려울 수 있습니다.` });
  } else {
    risks.push({ level: 'mid', title: '수익성 관리', desc: `${bt}의 변동비율은 약 ${Math.round(biz.vc * 100)}%입니다. ${biz.mainCost} 비용 절감이 수익성의 핵심입니다.` });
  }
  risks.push({
  ...
  const strategies = [];
  if (bep && bep <= 24) {
    strategies.push({ title: '초기 고정비 최소화 전략', desc: `BEP ${bep}개월 달성을 위해 초기 인테리어를 최소화하고 보증금보다 월세 절감에 집중하세요.` });
  } else {
    strategies.push({ title: '수익 다각화 전략', desc: `BEP 달성까지 ${bep || '다수'}개월이 예상됩니다. 배달·테이크아웃·온라인 판매 등 추가 수익 채널을 개설하세요.` });
  }
  strategies.push({ title: '소상공인 정책 자금 활용', desc: `소상공인진흥공단의 창업패키지(최대 1억원 지원) 및 시중은행 특별보증 상품을 활용하면 초기 투자 부담을 줄일 수 있습니다.` });
...
module.exports = async (req, res) => {
  ...
  // ── LLM 모드: ANTHROPIC_KEY 설정 시 Claude API 사용 ──
...
{
  "summary": "2~3문장 핵심 요약",
  "risks": [{"level":"high|mid|low","title":"리스크명","desc":"설명"}],
  "strategies": [{"title":"전략명","desc":"설명"}]
}`;
      ...
      const llmResult = await new Promise((resolve) => {
        ...
        reqLlm.on('error', () => resolve(null));
        reqLlm.on('timeout', () => { reqLlm.destroy(); resolve(null); });
        reqLlm.write(payload);
        reqLlm.end();
      });

      if (llmResult) return res.status(200).json({ ...llmResult, isMock: false });
    } catch (e) { /* fallback */ }
  }

  // ── Mock 모드 (기본) ──
  return res.status(200).json(
    generateMockSummary({ bizType, areaName, riskScore, roi, bep, competitionScore, regulationGrade, closeRate, survivalRate })
  );
};
